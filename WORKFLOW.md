# Learner: Knowledge Synthesis Pipeline

> **Collapsing the knowledge pipeline. Taking anyone to the cutting edge.**
>
> An AI-powered knowledge synthesis system that generates the exact book each person needs — tailored across infinite dimensions: your domain, your goals, your level, your language, your style. Using Google's Gemini models for intelligent synthesis, Synalinks for neuro-symbolic orchestration, and Gemini's image generation for custom covers and illustrations.

## Technology Stack

| Component | Technology | Purpose |
|-----------|------------|---------|
| **LLM Backbone** | Gemini 2.0 Flash | Content generation, planning, quality control |
| **Orchestration** | [Synalinks](https://github.com/SynaLinks/synalinks) | Neuro-symbolic pipelines with structured outputs |
| **Image Generation** | Gemini 2.0 Flash (image preview) | Book covers and concept illustrations |
| **Prompt Engineering** | Dynamic via Synalinks | Cover prompts generated based on book content |
| **PDF Generation** | WeasyPrint | Professional book formatting |

## Key Gemini Features Used

- **Structured Output**: All LLM calls use Synalinks DataModels for type-safe JSON responses
- **Multi-turn Reasoning**: Hierarchical planning (book → chapters → sections → subsections)
- **Temperature Control**: High temperature (1.0) for creative diversity, low for consistency
- **Image Generation**: Dynamic cover art and concept illustrations
- **Long Context**: Full book outline passed to planners for coherent narrative

## Overview

Learner collapses the decade-long knowledge pipeline — from breakthrough to accessible learning — into hours. We synthesize scattered knowledge from papers, docs, blogs, and cross-domain insights into a book built molecule by molecule for exactly one person.

The system creates comprehensive, deeply personalized books through a multi-stage pipeline that ensures coherent, well-structured content with minimal repetition.
...
**Process:**
1. **LLM Analysis**: Gemini analyzes chapter content to identify where illustrations would add value
2. **Mermaid Diagrams**: Generate flowcharts, sequence diagrams, class diagrams, state machines, mind maps
3. **AI Images**: Generate concept visualizations using Gemini's image generation

**Illustration Types:**
- `mermaid_flowchart` - Processes, workflows, decision trees
- `mermaid_sequence` - Interactions, communication patterns
- `mermaid_class` - Relationships, hierarchies, structures
- `mermaid_state` - State machines, lifecycle diagrams
- `mermaid_mindmap` - Concept relationships, topic overviews
- `generated_image` - Abstract concepts, visual metaphors

**Config:**
```yaml
enable_illustrations: true
enable_generated_images: true
image_model: "gemini/gemini-3-pro-image-preview"
```

**Files saved:**
- `04_illustrated_*.txt` - Illustrated chapter content
- `img_*.png` - Generated concept images
...
**Process:**
1. **Dynamic Prompt Generation** (via Synalinks): The cover prompt is generated by an LLM based on the book's actual content — topic, goal, audience, and chapter names. This ensures the cover visually represents THIS specific book, not generic imagery.

2. **Style Selection**: Choose from 15 artistic styles:
   - `humorous` - New Yorker editorial cartoon
   - `abstract` - Clean geometric, modern
   - `cyberpunk` - Neon, Love Death Robots style
   - `minimalist` - Ultra-clean, single focal point
   - `watercolor` - Soft, painterly
   - `vintage` - 19th century scientific illustration
   - `blueprint` - Technical drawing aesthetic
   - `surreal` - Dalí-esque dreamlike
   - `isometric` - 3D isometric technical art
   - `papercraft` - Paper cut-out layered style
   - `neon_noir` - Dark with neon accents
   - `botanical` - Nature-inspired scientific
   - `bauhaus` - Bold geometric Bauhaus
   - `pixel_art` - Retro 8-bit/16-bit
   - `art_deco` - 1920s glamorous

3. **Image Generation**: Gemini generates the cover image based on the dynamic prompt

4. **Text Overlay**: Title, subtitle, and author are added with professional typography

**Input:** Book metadata + full book context (topic, goal, chapters)

**Output:** PNG image suitable for book cover (3:4 aspect ratio)

**Config:**
```yaml
cover_style: "surreal"  # Any of the 15 styles above
image_model: "gemini/gemini-3-pro-image-preview"
```

**Files saved:**
- `book_cover.png` - Generated book cover image
- `cover_prompt.txt` - The dynamically generated prompt (for debugging)
...
```bash
pip install synalinks python-dotenv markdown weasyprint google-genai pillow
```
...
The `GEMINI_API_KEY` is used for:
- **LLM calls** via Synalinks/LiteLLM (Gemini 2.0 Flash)
- **Image generation** for covers and illustrations (Gemini image preview)
...
# 3. Generate a book (limited to 3 chapters for quick test)
python main.py --config configs/neurosymbolic.yaml --chapters 3

# 4. Generate full book
...
A typical book generation produces:
- **PDF**: Professional A4 book with cover, TOC, chapters
- **Markdown**: Full book content for further editing
- **Artifacts**: Plans, outlines, and intermediate content for transparency

Example: A 4-chapter book on "Neurosymbolic AI" generates ~50 pages of content in ~15 minutes.
